...
const float SENSOR_BACK_STEP = 195;

const float STEP_ANGLE = 0.144;
...
float _step_zero_angle = 180;  // ex) 50: 50 step == 0' //ST,0,INIT,0,ED\r\n   


int _step_delay = STEP_DELAY; //ST,0,MOVE,0,180,90,ED\r\n    
...
void motorStep(bool isClockwise, int steps) {
  for(int i=0; i<steps; i++) {
    ...
    //Serial.print("moterStep " +  String(_step_current_position) + "\r\n" );
    
    String cw = isClockwise?"CW":"CCW";
...
void moveMotor(int stepTarget, int speedPercent) {
  ...
  //Serial.print("moveMotor speed" + String(speed) + "\r\n" );

  _step_delay = speed;

  //int currentAngle = 0;
  int targetSteps = abs(stepTarget) / STEP_ANGLE;
  //bool isClockwise = stepTarget >= 0;

  bool isClockwise = (_step_current_position - stepTarget) <= 0;
  ...
  while (_step_current_position != targetSteps) {
    ...
    //Serial.print("moveMotor current step " + String(_step_current_position) +  String(_step_current_position * STEP_ANGLE) + "\r\n" );
...
void initializeMotor() {
  ...
  // loop whether check intial position or not
  do {
  ...
  } while(sleft == false && sright == false); //changed
...
void processCommand(String commandStr) {
  ...
  if (cmd.equals("MOVE")) {
    ...
    int step_target_position = ((float)angle / STEP_ANGLE) + _step_zero_angle;

    //Serial.print("test1 zero step" + String(_step_zero_angle) + "\r\n" );
    //Serial.print("test2 target step" + String(step_target_position) + "\r\n" );
    
    // Move motor to the target position at the specified speed
    moveMotor(step_target_position, speedPercent);

    Serial.print("test3 target step" + String(step_target_position) + "\r\n" );

    // Check if the motor has reached the target position and speed
    //bool isMotorAtTarget = (abs(angle - (step_target_position * STEP_ANGLE)) < 1) && (speedPercent == map(_step_delay, 1000, 5000, 0, 100));
